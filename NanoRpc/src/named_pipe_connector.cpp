...
NamedPipeConnector::NamedPipeConnector( const wchar_t *pipe_name, const wchar_t *server_name ) :
	is_connecting_( 0 ),
	callback_( NULL )
{
...
void NamedPipeConnector::StopConnection()
{
	stop_connection_.Set();
}


DWORD CALLBACK NamedPipeConnector::WaitForClientThreadProcThunk( LPVOID parameter )
{
...
DWORD CALLBACK NamedPipeConnector::WaitForServerThreadProcThunk( LPVOID parameter )
{
	assert( parameter != NULL );
	return reinterpret_cast<NamedPipeConnector *>( parameter )->WaitForServerThreadProc();
}
...
bool NamedPipeConnector::StartServerConnection()
{
	...
	if( err == ERROR_SUCCESS ) {
		...
		if( QueueUserWorkItem( &WaitForClientThreadProcThunk, state, WT_EXECUTELONGFUNCTION ) == FALSE ) {
			delete state;
			CancelIo( pipe );
			CloseHandle( pipe );
			InterlockedExchange( &is_connecting_, 0 );
			...
			// We do not change is_connecting_ here, because we are still pending the connection
...
bool NamedPipeConnector::StartClientConnection()
{
	...
	if( pipe != INVALID_HANDLE_VALUE ) {
		...
		if( QueueUserWorkItem( &WaitForServerThreadProcThunk, this, WT_EXECUTELONGFUNCTION ) == FALSE ) {
			InterlockedExchange( &is_connecting_, 0 );
			return false;
		}
	}
...
int NamedPipeConnector::WaitForClientThreadProc( HANDLE pipe )
{
	...
	while( true ) {
		...
		//WaitForSingleObject( overlapped_event_, INFINITE );   // TODO: should be WaitForMultipleObjects to wait for cancellation too.

		// And get the result
		DWORD bytes_transferred;
		if( GetOverlappedResult( pipe, &overlapped_, &bytes_transferred, TRUE ) == FALSE ) {
		...
		if( err == ERROR_SUCCESS ) {
			...
			// If call fails immediately, there is possiblity that the thread would go spinning.
...
HANDLE NamedPipeConnector::AttemptConnectToServer()
{
	...
	if( WaitNamedPipe( full_pipe_name.c_str(), NMPWAIT_USE_DEFAULT_WAIT ) != FALSE ) {
