...
public:
	...
	~ObjectPool()
	{
		...
		for( PointerToDescriptorMap::iterator iter = all_objects_.begin(); iter != all_objects_.end(); iter ++ ) {
	...
	T *Allocate()
	{
		...
		if( free_objects_.size() == 0 ) {
			...
			all_objects_.insert( PointerToDescriptorMap::value_type( descriptor->data, descriptor ) );
		}
		else {
	...
	size_t GetTotalObjectsCount() const
	{
		return all_objects_.size();
	}

	size_t GetFreeObjectsCount() const
	{
