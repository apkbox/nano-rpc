...
bool min_value_predicate( const std::pair<int,int> &a, const std::pair<int,int> &b )
{
...
BufferPool::BufferPool( unsigned int pool_size ) :
...
BufferPool::~BufferPool()
{
	...
	for( PointerToDescriptorMap::iterator iter = all_buffers_.begin(); iter != all_buffers_.end(); iter ++ ) {
...
void BufferPool::Deallocate( const char *buffer )
{
...
#if !defined( NDEBUG )
	// This is failproof way to check if the buffer is already in the pool, but it is slow.
	for( SizeToDescriptorMap::const_iterator iter = free_buffers_.begin(); iter != free_buffers_.end(); iter ++ ) {
	...
	// If there already enough free buffers, do not return the deallocated buffer - just delete it.
	// We keep usage statistics which buffer sizes are used the most and those which are kept in the pool.
	if( free_buffers_.size() >= pool_size_ ) {
		SizeToUsageMap::iterator iter = std::min_element( use_statistics_.begin(), use_statistics_.end(), min_value_predicate );
		...
		for( SizeToDescriptorMap::iterator iter = free_buffers_.find( buffer_size_to_erase ); iter != free_buffers_.end(); iter ++ ) {
	...
	free_buffers_.insert( free_buffers_.begin(), SizeToDescriptorMap::value_type( buffer_descriptor->size, buffer_descriptor ) );
}
...
int BufferPool::GetBufferSize( const char *buffer ) const
{
	...
	assert( iter != all_buffers_.end() );
	if( iter == all_buffers_.end() )
		throw std::exception( "BufferPool::GetBufferSize: buffer does not belong to this pool or buffer pointer is invalid." );

	// Check if someone attempts to fool us by providing buffer that is free but still in the pool.
...
BufferPool::Descriptor *BufferPool::CreateBuffer( int size )
{
	...
	descriptor->used = true;
	all_buffers_.insert( PointerToDescriptorMap::value_type( descriptor->data, descriptor ) );
	return descriptor;
}
