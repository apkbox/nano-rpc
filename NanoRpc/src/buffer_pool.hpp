...
class BufferPool
{
	struct Descriptor
	{
		char *data;
		int size;
		bool used;   // This only needed to optimize the lookup when deallocating the buffer.
	};

	// There is a apparent bug in hash_map, so we use map instead, although hash_map would be much better.
	...
	static const unsigned int DefaultPoolSize = 32;      // Maximum number of free buffers in the pool
...
public:
	...
	void set_pool_size( unsigned int pool_size )
	{
		pool_size_ = pool_size;
	}

	// Allocates a new buffer of the specified size. The actual buffer size may be bigger.
	...
	// If the buffer does not belong to the pool or an invalid pointer is passed, then
	// exception is thrown.
