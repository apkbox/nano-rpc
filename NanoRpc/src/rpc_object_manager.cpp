...
const char *const RpcObjectManager::ServiceName = "NanoRpc.ObjectManagerService";
...
RpcObjectManager::~RpcObjectManager()
{
	for( std::map<RpcObjectId, IRpcService *>::const_iterator iter = objects_.begin(); 
...
{
	std::map<std::string, RpcObjectId>::const_iterator iter = services_.find( name );
...
IRpcService *RpcObjectManager::GetInstance( RpcObjectId object_id )
{
	...
	std::map<RpcObjectId, IRpcService *>::const_iterator iter = objects_.find( object_id );
...
void RpcObjectManager::CallMethod( const RpcCall &rpc_call, RpcResult *rpc_result )
{
	...
	if( rpc_call.method() == "Delete" ) {
		...
		if( rpc_call.parameters_size() != 1 || !rpc_call.parameters().Get(0).has_uint32_value() ) {
