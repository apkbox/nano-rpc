...
public:
	...
	NamedPipeConnector();
	NamedPipeConnector( const wchar_t *pipe_name, const wchar_t *server_name = L"." );
	...
	//
	// First it tries to do connection synchronously. If connection established, then callback
	...
	// is called from pool thread and once callback returns, the thread terminates.
	// This has a consequence that any overlapped operation started in the callback
	// would fail.
	...
	//
	// If connection is already pending and specified connection side is same as of
