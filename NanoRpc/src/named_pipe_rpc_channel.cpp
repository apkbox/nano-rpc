...
NamedPipeRpcChannel::NamedPipeRpcChannel( RpcController *controller, HANDLE pipe_handle ) :
	RpcChannel( controller ),
	pipe_( pipe_handle ),
	completion_port_( NULL ),
	completion_thread_id_( 0 ),
	completion_thread_( NULL ),
	disconnected_callback_( NULL ),
...
{
	if( InterlockedCompareExchange( &is_connected_, Connected, NotConnected ) == NotConnected ) {
		...
		completion_thread_ = CreateThread( NULL, 0, &IoCompletionThreadProcThunk, this, CREATE_SUSPENDED, &completion_thread_id_ );
		...
		if( CreateIoCompletionPort( pipe_, completion_port_, ( ULONG_PTR )pipe_, 0 ) == NULL ) {
			CloseHandle( completion_port_ );
			InterlockedExchange( &is_connected_, NotConnected );
			return false;
		}
		...
		if( ResumeThread( completion_thread_ ) == ( DWORD )-1 ) {
			// This is fatal condition. We cannot restart it again, because the pipe is already associated with
			// completion port, so, we cannot retry it.
			InterlockedExchange( &is_connected_, Disconnected );
			return false;
		}

		// We should not attempt to start asynchronous read here because the thread that 
...
void NamedPipeRpcChannel::Send( const RpcMessage &message )
{
	...
	Overlapped *overlapped = AllocateOverlappedState( message.ByteSize() + sizeof( __int32 ) );

	*( __int32 * )overlapped->buffer = message.ByteSize();
	if( !message.SerializeToArray( overlapped->buffer + sizeof( __int32 ), message.ByteSize() ) )
		assert( false ); // TODO: return proper error to the upper level in release

	overlapped->operation_ = OverlappedOperation::Write;
	if( WriteFile( pipe_, overlapped->buffer, message.ByteSize() + sizeof( __int32 ), NULL, overlapped ) == FALSE ) {
		// TODO: Handle ERROR_OPERATION_ABORTED when CancelIOEx implementation added to disconnect.
...
void NamedPipeRpcChannel::StartRead( int expected_size, OverlappedOperation::Type requested_operation )
{
	if( is_connected_ != Connected )
		return; // TODO: false;
	...
	if( ReadFile( pipe_, overlapped->buffer, expected_size, NULL, overlapped ) == FALSE ) {
		// TODO: Handle ERROR_OPERATION_ABORTED when CancelIOEx implementation added to disconnect.
		// This would not be a surprise disconnect though.
...
void NamedPipeRpcChannel::ReadOperationCompleted( Overlapped *overlapped, DWORD bytes_read )
{
	...
	if( overlapped->operation_ == OverlappedOperation::ReadPrefix ) {
		...
		// Start next read before we process the message, so we don't have to wait for Receive to handle current message.
		// TODO: Thie only problem with this is that we still blocking the IoCompletionThreadProc.
		// So we get a lockup if client (native) receives event and attempts to make a synchronous call.
		// This precisely problem does not occur in the .NET channel, because thread pool is used to handle I/O.
		StartRead( sizeof( __int32 ), OverlappedOperation::ReadPrefix );
		...
		// Starting another read before Receive finished may also improve performance in case
...
void NamedPipeRpcChannel::WriteOperationCompleted( Overlapped *overlapped, DWORD bytes_written )
{
	//std::cout << "Write operation " << overlapped->operation_ << " completed\n";
	//std::cout.flush();
	FreeOverlappedState( overlapped );
}
...
int NamedPipeRpcChannel::IoCompletionThreadProc()
{
	...
	while( is_connected_ == Connected ) {
		...
		if( GetQueuedCompletionStatus( completion_port_, &bytes_transferred, &key, ( OVERLAPPED ** )&overlapped, INFINITE ) == FALSE ) {
			std::cout << "Overlapped operation failed: " << GetLastError() << "\n";

			// If overlapped is NULL then GetQueuedCompletionStatus failed because of invalid 
			// parameter or completion_port_ handle was closed.
			// This would also happen if timeout parameter is not INFINITE (which is not the case here).
			if( overlapped != NULL )
				FreeOverlappedState( overlapped );

			// TODO: We possibly need to handle ERROR_OPERATION_ABORTED when CancelIOEx implementation added to disconnect.
			// This would not be a surprise disconnect though.
			if( GetLastError() == ERROR_BROKEN_PIPE ) {
...
HANDLE NamedPipeRpcChannel::CloseConnection()
{
	...
	while( overlapped_pool_.GetTotalObjectsCount() - overlapped_pool_.GetFreeObjectsCount() > 0 ) {
		DWORD bytes_transferred;
		ULONG_PTR key;
		Overlapped *overlapped = NULL;

		if( GetQueuedCompletionStatus( completion_port_, &bytes_transferred, &key, ( OVERLAPPED ** )&overlapped, 0 ) == FALSE ) {
			if( overlapped != NULL ) {
				FreeOverlappedState( overlapped );
			}
			else {
				// If the previous call failed, we wait a bit, then we check if any objects
				// were freed since the last failure. If some objects were freed,
				// then we reset number of attempts and still keep trying.
				// If no object were freed, then we so few more attempts before bailing out.
				Sleep(500);
				...
				if( last_free_object_count < overlapped_pool_.GetFreeObjectsCount() ) {
					...
					if( --attempts == 0 ) {
						int used = overlapped_pool_.GetTotalObjectsCount() - overlapped_pool_.GetFreeObjectsCount();
						std::cout << "warning: Failed to release overlapped buffers. " << used << " buffers are still appears to be used.\n";
						break;
					}
				}
			}
...
{
	// We can get here from multiple points, but only one should be allowed to do the work.
	if( InterlockedCompareExchange( &is_connected_, Disconnected, Connected ) == Connected ) {
