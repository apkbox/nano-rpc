...
public:
  ...
  operator WAITORTIMERCALLBACK() { return CallbackThunk; }

  WAITORTIMERCALLBACK get_callback_ptr() { return *this; }
...
public:
  AsyncCallback(Object_ *object, Method_ method, const State_ &state)
      : object_(object), method_(method), state_(state) {}

  State_ &State() { return state_; }

  // TODO: This is temporary to troubleshoot some weirdness in overlapped I/O.
  void Invoke() { CallbackHandler(this, true); }

private:
  virtual void CallbackHandler(void *ptr, BOOLEAN timer_or_wait_fired) {
    Self_ *async = reinterpret_cast<Self_ *>(ptr);

    // We make local copy here, because the callback object can be deleted in
    // the callback method,
    // thus invalidating state parameter. It is not apparent for the callback
    // method implementer
    // that state is held in the AsyncCallback object.
    State_ state_copy = state_;
    ((async->object_)->*
     (async->method_))(static_cast<AsyncCallbackBase *>(async), state_copy);
  }
...
template <class Object_, typename Method_, typename State_>
AsyncCallback<Object_, Method_, State_> *
CreateAsyncCallback(Object_ *object, Method_ method, const State_ &state) {
  return new AsyncCallback<Object_, Method_, State_>(object, method, state);
}

class ThreadPool {
public:
  static bool RegisterAsyncIOOperation(HANDLE wait_handle,
